	...
	output_index_3(-1)
{
	...
	NV_CUDA_CHECK(cudaMalloc(&bindings[output_index_2], (size_t)batch_size * config->max_detection * sizeof(float) * 4));
	...
	NV_CUDA_CHECK(cudaMallocHost(&trt_output_buffers[1], (size_t)batch_size * config->max_detection * sizeof(float) * 4));
...
void darknet::YoloV3NMS::infer(const unsigned char* input)
{
	...
	NV_CUDA_CHECK(cudaMemcpyAsync(trt_output_buffers[1], bindings[output_index_2], (size_t)batch_size * config->max_detection * sizeof(float) * 4, cudaMemcpyDeviceToHost, cuda_stream));
