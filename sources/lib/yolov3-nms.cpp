...
void darknet::YoloV3NMS::infer(const unsigned char *input) {
    NV_CUDA_CHECK(
            cudaMemcpyAsync(bindings[input_index], input, (size_t) batch_size * config->INPUT_SIZE * sizeof(float),
    ...
    NV_CUDA_CHECK(cudaMemcpyAsync(trt_output_buffers[0], bindings[output_index_1],
                                  (size_t) batch_size * config->max_detection * sizeof(float), cudaMemcpyDeviceToHost,
                                  cuda_stream));
    NV_CUDA_CHECK(cudaMemcpyAsync(trt_output_buffers[1], bindings[output_index_2],
                                  (size_t) batch_size * config->max_detection * sizeof(float4), cudaMemcpyDeviceToHost,
                                  cuda_stream));
