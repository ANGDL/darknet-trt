...
darknet::YoloV3::YoloV3(NetConfig *config,
    ...
    convert_to_bboxes = Tensor2BBoxes(net_cfg->OUTPUT_CLASSES, net_cfg->BBOXES, net_cfg->ANCHORS, net_cfg->INPUT_W,
                                      net_cfg->INPUT_H);
    //分配内存
...
void darknet::YoloV3::infer(const unsigned char *input) {
    NV_CUDA_CHECK(
            cudaMemcpyAsync(bindings[input_index], input, (size_t) batch_size * net_cfg->INPUT_SIZE * sizeof(float),
                            cudaMemcpyHostToDevice, cuda_stream));
    context->enqueue(batch_size, bindings.data(), cuda_stream, nullptr);
    NV_CUDA_CHECK(cudaMemcpyAsync(trt_output_buffers[0], bindings[output_index_1],
                                  (size_t) batch_size * net_cfg->OUTPUT_SIZE_1 * sizeof(float), cudaMemcpyDeviceToHost,
                                  cuda_stream));
    NV_CUDA_CHECK(cudaMemcpyAsync(trt_output_buffers[1], bindings[output_index_2],
                                  (size_t) batch_size * net_cfg->OUTPUT_SIZE_2 * sizeof(float), cudaMemcpyDeviceToHost,
                                  cuda_stream));
    NV_CUDA_CHECK(cudaMemcpyAsync(trt_output_buffers[2], bindings[output_index_3],
                                  (size_t) batch_size * net_cfg->OUTPUT_SIZE_3 * sizeof(float), cudaMemcpyDeviceToHost,
                                  cuda_stream));
