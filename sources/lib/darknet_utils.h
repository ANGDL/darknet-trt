...
struct Tensor2BBoxes {
    ...
    std::vector<BBoxInfo> operator()(const float *detections, const std::vector<int> mask, const unsigned int gridSize,
                                     const unsigned int stride, const float confidence_thresh, const int raw_w,
                                     const int raw_h);
